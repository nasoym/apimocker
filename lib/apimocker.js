...
apiMocker.logRequest = function(req, res, options) {
  if (options.log && options.log=="detailed") {
    apiMocker.log("Request: " +
        req.method + " : " + req.url );
    if (req.headers) {
        apiMocker.log("    headers: " + JSON.stringify(req.headers));
    }
    if (req.query) {
        apiMocker.log("    query: " + JSON.stringify(req.query));
    }
    if (req.body) {
        apiMocker.log("    body: " + JSON.stringify(req.body));
    }
    apiMocker.log("Returning mock: " + options.mockFile);
    apiMocker.log("    status: " + options.httpStatus);
    if (options.headers) {
        apiMocker.log("    headers: " + JSON.stringify(options.headers));
    }
  } else if (options.log && options.log=="none") {
  } else {
...
apiMocker.createServer = function(options) {
  ...
  apiMocker.expressLogger = function(req, res, next) {
      apiMocker.log("---> Request: " + req.method + " : " + req.url + " : " + JSON.stringify(req.headers));
      next();
  }
  ...
  if (apiMocker.options.detailed_log) {
    apiMocker.express.use(apiMocker.expressLogger);
  }
...
apiMocker.sendResponse = function(req, res, serviceKeys) {
  ...
  setTimeout(function() {
    apiMocker.logRequest(req, res, options); 
